...
class log(object):
    ...
    def deserialize(serialization):
        log_type, timestamp, rest = deserialize_parts(serialization)
        return log(log_type, timestamp, rest)
...
class bitcoin_log(log): # bitcoin connection/disconnection events
    def repack(self):
        first = pack('>I', self.handle_id)
        second = pack('=hHI8xhHI8x', socket.AF_INET, self.r_port_, self.r_addr_, socket.AF_INET, self.r_port_, self.r_addr_)
    ...
    def deserialize(timestamp, rest):
        ...
        fam1, r_port_, r_addr_, fam2, l_port_, l_addr_ = unpack('=hHI8xhHI8x', rest[8:16])
        text_len, text = unpack('>Is', rest[16:])
        return bitcoin_log(log_types.BITCOIN, timestamp, handle_id, update_type, 
                           socket.inet_ntoa(r_addr_), socket.ntohs(r_port_), 
                           socket.inet_ntoa(l_addr_), socket.ntohs(l_port_), text)
...
class bitcoin_msg_log(log):
    ...
    def deserialize(timestamp, rest):
        ...
        return bitcoin_msg_log(log_types.BITCOIN_MSG, timestamp, handle_id, is_sender, rest[6:])
