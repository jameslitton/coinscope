#ifndef ALLOC_BUFFER_HPP
#define ALLOC_BUFFER_HPP
...
/* This is for stuff too small to use mmap_buffer ~100k? */

template <typename T>
class alloc_buffer { /* this is an efficient COW buffer using mmap */
public:
...
public:
	...
	struct iterator { /* non-const iterator has to COW, so blech */
		...
		typedef typename alloc_buffer<T>::difference_type difference_type;
		typedef typename alloc_buffer<T>::value_type value_type;
		typedef typename alloc_buffer<T>::pointer pointer;
