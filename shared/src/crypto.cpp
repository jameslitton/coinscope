...
/* taken from http://bradconte.com/sha256_c. Just so I don't have to
   use ssl with the static build */
...
unique_ptr<unsigned char[]> sha256(const uint8_t *data, size_t len) {
	...
	unique_ptr<unsigned char[]> rv(new unsigned char[32]);

	sha256_update(&ctx, data, len);
	sha256_final(&ctx, rv.get());
